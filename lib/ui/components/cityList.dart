  ...
  Widget build(BuildContext context) {
    if (isMoveing != true) {
      return FutureBuilder(
        future: isSelectCity
            ? placeBloc.getplaces(cityName, searchType)
            : placeBloc.searchPlaces(capitalize(widget.inputValue), 'city'),
          ...
          if (results.hasData) {
            return Expanded(
              child: Stack(
                children: [
                  ListView.builder(
                    itemCount: (results.data as List?)?.length ?? 0,
                    itemBuilder: (context, index) {
                      final searchRe = (results.data as List?)?[index];
                              ...
                              if (isSelectCity == false) {
                                ...
                                setState(() {
                                  ...
                                  if ((selectedPlaces.any(
                                    (itemToCheck) => itemToCheck.id == placeId,
                                  ))) {
                                    selectedPlaces.removeWhere(
                                      (item) => item.id == placeId,
                                    );
                                  } else {
                                    selectedPlaces.add(
                                      (results.data as List)[index],
                                    );
                                  }
                                });
                              }
