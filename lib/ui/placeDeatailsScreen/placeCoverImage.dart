                          ...
                          return InkWell(
                            onTap: () async {
                              if (favSnapshot.data[0] == false) {
                                BlocProvider.of<placeListBloc>(context).add(placeAddToFavorites(
                                  atPlaceId: placeDetails.id,
                                  placeName: placeDetails.name,
                                  placeImgUrl: placeDetails.photoRef,
                                  type: placeDetails.type,
                                ));
                                setState(() {
                                  favSnapshot.data[0] = true;
                                });
                                // Có thể hiển thị SnackBar: Đã thêm vào yêu thích
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(content: Text("Đã thêm vào danh sách yêu thích")),
                                );
                              } else {
                                BlocProvider.of<placeListBloc>(context).add(placeRemoveFromFavorites(
                                  atPlaceId: placeDetails.id,
                                ));
                                setState(() {
                                  favSnapshot.data[0] = false;
                                });
                                // Có thể hiển thị SnackBar: Đã xóa khỏi yêu thích
                                ScaffoldMessenger.of(context).showSnackBar(
